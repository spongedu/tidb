...
func (iter *LogIterator) carpet(bpos, epos int64, point time.Time) error {
	...
	for pos < epos {
		...
		//logrus.Infof("file=%s | item.ts=%s | point=%s | iter.end=%s", item.GetFileName(), item.GetTime().String(), point.String(), iter.end.String())
		//logrus.Infof("item.GetTime().After(point)=%s", item.GetTime().After(point))
		//logrus.Infof("item.GetTime().After(iter.end)=%s", item.GetTime().After(iter.end))
